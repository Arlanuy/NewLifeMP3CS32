...
void copyEdges(Edge** dest_edge_list, Edge** src_edge_list, int edge_trav_iter) {
    int i;
    for(i = 0; i < edge_trav_iter ; i++) {
        if ((dest_edge_list[i] != NULL) && (src_edge_list[i] != NULL)) {
            dest_edge_list[i] = src_edge_list[i];
        }
    }
}

int allColorGray(int num_of_vertex, int except_this, int h, int j, int k) {
    int i, count;

    for (i = 1; i <= num_of_vertex; i++) {
        if ((i != except_this) && (strcmp(color[i - 1], "gray") != 0)) {
            printf("i is %d and j is %d and k is %d ", h, j, k);
...
int DFS(FILE* output_file, int num_of_vertex, GRAPH* graph, Edge** longest_path, Edge** edge_list, int i, int* travelled_destination, Travel_Stats travel_stats, int start_vertex, int end_vertex) {
    Node* alpha;
    int color_white_final_vertex = TRUE, traverse_list = TRUE, traverse_list_gray = TRUE;
    ...
    else if ((start_vertex == end_vertex) && (i == end_vertex)) {
        color_white_final_vertex = FALSE;
        *travelled_destination = TRUE;
    ...
    else if (allColorGray(num_of_vertex, end_vertex, i, end_vertex, travel_stats.edge_trav_iter) == TRUE) {
        color_white_final_vertex = FALSE;
        *travelled_destination = TRUE;
        traverse_list_gray= FALSE;
        traverse_list = FALSE;
    ...
    while (alpha != NULL) {
        ...
        if (alpha != NULL) {
            if (traverse_list == TRUE) {
                ...
                for(p = 0; p < travel_stats.edge_trav_iter ; p++) {
                    assert(new_travel_stats.edge_list[p] != NULL);
                    assert(travel_stats.edge_list[p] != NULL);
                    new_travel_stats.edge_list[p] = travel_stats.edge_list[p];
                }*/
                copyEdges(new_travel_stats.edge_list, travel_stats.edge_list, travel_stats.edge_trav_iter);
        ...
        if ((traverse_list == TRUE) && (strcmp(color[j - 1], "white") == 0)) {
            ...
            DFS(output_file, num_of_vertex, graph, longest_path, edge_list, j, travelled_destination, new_travel_stats, start_vertex, end_vertex);
        }
            ...
            if ((start_vertex == end_vertex) && (end_vertex == j)) {
                ...
                //edge->finish = j;
                //edge->TRAVEL_COST =
                //printf("pointer %p\n", edge_list[(i - 1) * (num_of_vertex) + (j - 1)]);
                //printf("with cost %d\n", edge_list[((i - 1) * (num_of_vertex)) + (j - 1)]->TRAVEL_COST);
                //int o, p;
                /**
                for (o = 0; i < num_of_vertex; o++) {
                    for (p = 0; p < num_of_vertex; p++) {
                        if (edge_list[(o  * num_of_vertex) + p] != NULL) {
                            printf("%d-%d with cost: %d\n", edge_list[(o  * num_of_vertex) + p]->start, edge_list[(o  * num_of_vertex) + p]->finish, edge_list[(o  * num_of_vertex) + p]->TRAVEL_COST);
                        }
                    }
                    printf("\n");
                }*/
                Edge* edge = edge_list[((i - 1) * (num_of_vertex)) + (j - 1)];
                //0printf("magic i is %d and j is %d with edge cost %d\n", i, j, edge->TRAVEL_COST);
                new_travel_stats.TOTAL_TRAVEL_COST += edge->TRAVEL_COST;
                new_travel_stats.edge_list[new_travel_stats.edge_trav_iter] = edge;
                (new_travel_stats.edge_trav_iter)++;
            ...
            case 0:
             ...
             case 1:
                ...
                if ((start_vertex != 0 && (start_vertex <= num_of_vertex) && (start_vertex >= 1)) &&
                    ...
                    int vertex_trav_iter, vertex_trav_iter2, color_iter;

                    //using the cost adjacency matrix to build up every element of node_list by appending adjacent vertex

                    if (edge_list  != NULL) {
                        int m, n;
                        for (m = 0; m < num_of_vertex; m++) {
                            for (n = 0; n < num_of_vertex; n++) {
                                if (edge_list[(m * num_of_vertex) + n] != NULL) {
                                    free(edge_list[(m * num_of_vertex) + n]);
                                }
                    ...
                    for (vertex_trav_iter = 1; vertex_trav_iter <= num_of_vertex; vertex_trav_iter++) {
                       ...
                       for (vertex_trav_iter2 = 1; vertex_trav_iter2 <= num_of_vertex; vertex_trav_iter2++) {
                            if (cost_adj_mat[vertex_trav_iter - 1][vertex_trav_iter2 - 1] != 1000 && cost_adj_mat[vertex_trav_iter - 1][vertex_trav_iter2 - 1] != 0) {
                                ...
                                edge_list[((vertex_trav_iter - 1) * (num_of_vertex)) + (vertex_trav_iter2 - 1)] = NULL;
                            }
                    ...
                    travel_stats.edge_list = malloc((num_of_vertex - 1) * sizeof(Edge*));
                    ...
                    cost_longest_path = 0;
                    Edge** longest_path = malloc((num_of_vertex - 1) * sizeof(Edge*));
                    DFS(output_file, num_of_vertex, graph, longest_path, edge_list, start_vertex, &travelled_destination, travel_stats, start_vertex, end_vertex);
                    free(travel_stats.edge_list);
                    ...
                    if (travelled_destination == TRUE) {
                        ...
                        for (edge_iter = 0; edge_iter < max_edge_trav_iter; edge_iter++) {
                            if (longest_path[edge_iter] != NULL) {
                                printf("%d-%d with cost %d\n", longest_path[edge_iter]->start, longest_path[edge_iter]->finish, longest_path[edge_iter]->TRAVEL_COST);
                                if (edge_iter == max_edge_trav_iter - 1) {
                                    printf("%d\n", longest_path[edge_iter]->finish);
                                }
                            }
